#!/usr/bin/env python
"""Re-evaluate existing v1 Independent INR best.pt under common eval mask.

Evaluation-only. Does NOT retrain. Does NOT overwrite best.pt.
...
def _legacy_dwi(summary_csv: Path) -> dict[str, float]:
    if not summary_csv.is_file():
        return {}
    with open(summary_csv, newline="", encoding="utf-8") as f:
        return {r["subject_id"]: float(r["DWI_RelMSE"]) for r in csv.DictReader(f)}


def main() -> None:
    ap = argparse.ArgumentParser(description="Common-mask re-eval of existing best.pt (no retrain)")
    ...
    legacy = _legacy_dwi(exp / "summary.csv")

    if args.subjects.strip():
        sids = [s.strip() for s in args.subjects.split(",") if s.strip()]
    else:
        sids = sorted(
            p.parent.name
            for p in exp.glob("*/best.pt")
            if p.parent.name.isdigit() or len(p.parent.name) == 6
        )
        # Prefer summary order if available
        if (exp / "summary.csv").is_file():
            with open(exp / "summary.csv", newline="", encoding="utf-8") as f:
    ...
    for i, sid in enumerate(sids, 1):
        ...
        if n_common == 0:
            raise RuntimeError(f"{sid}: empty common_mask — stop without changing thresholds")
        ...
        # Preserve training stats from original subject metrics if present
        old_metrics = exp / sid / "metrics.json"
        training = {
        ...
        if old_metrics.is_file():
            ...
            tr = old.get("training", {})
            training.update({k: tr.get(k, training.get(k)) for k in training})
        ...
        metrics_obj = build_metrics_json(
            ...
            extra={
                ...
                "reeval_only": True,
                "legacy_brain_DWI_RelMSE": legacy.get(sid),
                "n_brain_voxels": n_brain,
                "n_wls_valid_voxels": n_wls_valid,
                "n_common_voxels": n_common,
                "training_mask": "brain (unchanged; not used in this re-eval)",
                "evaluation_mask": "brain & WLS_valid",
            },
        )
        # Do not overwrite original best.pt — save metrics/maps only under eval_common_mask/
        out_dir = out_root / sid
        out_dir.mkdir(parents=True, exist_ok=True)
        from inr.io_utils import save_json
        ...
        rows.append(row)
        old_dwi = legacy.get(sid, float("nan"))
        print(
            f"  FA_MAE={row['FA_MAE']:.4f}  DWI_RelMSE={row['DWI_RelMSE']:.6e}  "
            f"legacy_brain_DWI={old_dwi:.6e}  n_eval={row.get('n_eval_voxels')}  "
            f"n_sampled={row.get('n_sampled_voxels')}"
        )
    ...
    print(f"\n[reeval] wrote {out_root / 'summary.csv'}")
    print(f"[reeval] wrote {out_root / 'aggregate.md'}")
